...
const DocumentCard = ({ document, onDelete}) => {
    ...
    return <div
     className="group relative bg-white/80 backdrip-blur-xl border border-slate-200/60 rounded-2xl p-5 hover:border:slate-300/60 hover:shadow-xl hover:shadow-slate-200/50 transition-all duration-300 flex flex-col justify-between cursor-pointer hover:-translate-y-1"
     onClick={handleNavigate}
    >
        {/* Header Section */}
        <div>
            <div className="flex items-start justify-between gap-3 mb-4">
                <div className="shrink-0 w-12 h-12 bg-linear-to-br from-emerald-500 to-cyan-500 rounded-xl flex items-center justify-center shadow-lg shadow-blue-500/25 group-hover:scale-110 transition-tansform duration-300">
        ...
        <div className="mt-5 pt-4 border-t border-slate-100">
            <div className="flex items-center gap-1.5 text-xs text-slate-500">
                ...
                <span>Uplaoded {moment(document.createdAt).fromNow()}</span>
            </div>
        </div>
